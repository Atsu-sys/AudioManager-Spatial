...
public class AudioManagerSetting : ScriptableObject {
  ...
  public  static AudioManagerSetting  Entity{
    get{
      if(_entity == null){
        // まずResourcesフォルダから読み込みを試みる
        _entity = Resources.Load<AudioManagerSetting>("AudioManagerSetting");
        
        #if UNITY_EDITOR
        // Resourcesで見つからなければ、AssetDatabaseで検索（エディタのみ）
        if(_entity == null){
          string[] guids = UnityEditor.AssetDatabase.FindAssets("t:AudioManagerSetting");
          if(guids.Length > 0){
            string path = UnityEditor.AssetDatabase.GUIDToAssetPath(guids[0]);
            _entity = UnityEditor.AssetDatabase.LoadAssetAtPath<AudioManagerSetting>(path);
          }
        }
        
        // それでも見つからなければ、デフォルトの設定を持つインスタンスを作成
        if(_entity == null){
          _entity = CreateInstance<AudioManagerSetting>();
          Debug.LogWarning("AudioManagerSetting が見つかりません。デフォルト設定を使用します。\n" +
                          "Assets/Resources/ フォルダに AudioManagerSetting.asset を作成することを推奨します。");
        }
        #else
        // ランタイムでResourcesに無い場合はデフォルト設定を使用
        if(_entity == null){
          _entity = CreateInstance<AudioManagerSetting>();
          Debug.LogWarning("AudioManagerSetting が Assets/Resources/ に見つかりません。デフォルト設定を使用します。");
        }
        #endif
      }
      return _entity;
